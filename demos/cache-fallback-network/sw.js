...
self.addEventListener('install', function(event) {
  console.log('fallback network', version, 'now installing...');
...
self.addEventListener('activate', function(event) {
  console.log('fallback network', version, 'now ready to handle fetches!');
...
self.addEventListener('fetch', event => {
  console.log('fallback network', version, 'linsten a fetch event', event.request.url);

  // respond from caches first and network as a backup way
  event.respondWith(
    caches.open('pwa-studio').then(function(cache) {
